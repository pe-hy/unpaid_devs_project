...
//            FileWriter outputfile = new FileWriter(file);
            ...
            String[] header = {"Předmět", "Datum", "Od", "Do", "Škola", "Učitel", "Studenti" };
            writer.writeNext(header);

            for (Practice p :
                    practices) {
                List<User> students = p.getStudents();
                String[] data = {p.getSubject().getName(), p.getDate().toString(), p.getStart().getHour() + ":" + (p.getStart().getMinute() == 0 ? "00" : p.getStart().getMinute()) , p.getEnd().getHour() + ":" + (p.getEnd().getMinute() == 0 ? "00" : p.getEnd().getMinute()), p.getTeacher().getSchool().getName(), p.getTeacher().getFirstName() + " " + p.getTeacher().getSecondName() + " (" + p.getTeacher().getUsername() + ")", students.size() > 0 ? students.get(0).getFirstName() + " " + students.get(0).getSecondName() + " (" + students.get(0).getUsername() + ")" : "---"};
