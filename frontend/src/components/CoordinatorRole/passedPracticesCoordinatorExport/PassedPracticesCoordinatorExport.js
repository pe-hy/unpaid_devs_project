...
export const PassedPracticesCoordinatorExport = () => {
    ...
    const downloadExport = async () => {
        ...
        const response = await axios({
            ...
            method: "POST",
            data: { "startYear": start.getFullYear(), "startMonth": start.getMonth() + 1, "startDay": start.getDate(), "endYear": end.getFullYear(), "endMonth": end.getMonth() + 1, "endDay": end.getDate()},
        }).catch((err) => {
        ...
        if (response && response.data) {
            ...
            link.href = url;
            link.setAttribute('download', `Export_praxí_${new Date().getDate()}/${new Date().getMonth()+1}/${new Date().getFullYear()}.csv`);
    ...
    function setDateRangeLimit(practices) {
        ...
        setDateRange([
            {
                startDate: addDays(lowestDate, -1),
                endDate: addDays(highestDate, 1),
                key: 'selection'
            }
        ]);
    }
